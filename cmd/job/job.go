...
func NewCmd() *cobra.Command {
	jobCmd := &cobra.Command{
		Use:   "job",
		Short: "Run the Rabbit job workers",
		Long:  `Run the Rabbit service with job workers only (EventBus for message processing)`,
		Annotations: map[string]string{
			"group": cmd.ServiceCommands,
		},
		Run: runJob,
	}
	flags.addFlags(jobCmd)
	return jobCmd
}

func runJob(_ *cobra.Command, _ []string) {
	...
	envOpts := []hello.Option{
		...
		hello.WithName(serverConf.GetName() + "-job"),
...
func newApp(d *data.Data, eventBus *server.EventBus, bc *conf.Bootstrap, helper *klog.Helper) (*kratos.App, error) {
	defer hello.Hello()
	opts := []kratos.Option{
		kratos.Logger(helper.Logger()),
		kratos.Server(eventBus),
