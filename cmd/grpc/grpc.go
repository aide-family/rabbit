// Package grpc provides the grpc-only command for the Rabbit service.
package grpc
...
func NewCmd() *cobra.Command {
	grpcCmd := &cobra.Command{
		Use:   "grpc",
		Short: "Run the Rabbit gRPC server only",
		Long:  `Run the Rabbit service with gRPC server only (without HTTP and job workers).`,
		Annotations: map[string]string{
			"group": cmd.ServiceCommands,
		},
		Run: runGRPC,
	}
	flags.addFlags(grpcCmd)
	return grpcCmd
}
...
func runGRPC(_ *cobra.Command, _ []string) {
	...
	envOpts := []hello.Option{
		...
		hello.WithName(serverConf.GetName() + "-grpc"),
